...
function [t w] = PredictorSis4Fun(w, t, h, N, numEcu, F)
  ...
  for k = 1:1:N  # este for nos permite agregar mas valores para mayores tiempos t con el mismo paso h
      ...
      for i = 1:1:numEcu
         j = length(w);
         f = F{i};
         aux(i) = w(i,j) + (h/24 * [55*f(t(j), w(1,j), w(2,j), w(3,j), w(4,j)) - 59*f(t(j-1), w(1,j-1), w(2,j-1), w(3,j-1), w(4,j-1)) + 37*f(t(j-2), w(1,j-2), w(2,j-2), w(3,j-2), w(4,j-2)) - 9*f(t(j-3), w(1,j-3), w(2,j-3), w(3,j-3), w(4,j-3) )]);
